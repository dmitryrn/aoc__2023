...
var (
	...
	strNumsReversed = func() []string {
		result := []string{}
		for _, strNum := range strNums {
			result = append(result, reverse(strNum))
		}
		return result
	}()
)
...
func main() {
	...
	for _, line := range lines {
		n1 := getFirstNum(line, strNums)
		n2 := getFirstNum(reverse(line), strNumsReversed)
		n1s := strconv.FormatInt(int64(n1), 10)
		n2s := strconv.FormatInt(int64(n2), 10)
		parsed, err := strconv.ParseInt(n1s+n2s, 10, 64)
		if err != nil {
			panic("insane")
		}
		sum += parsed
...
func getFirstNum(str string, strNums []string) int {
	digit := 0
	digitIdx := -1
	for i, c := range str {
		n := c - 48
	...
	minIdx := 999999999999999999
	minN := 999999999999999999
	for i, strNum := range strNums {
		idx := strings.Index(str, strNum)
		if idx > -1 && idx < minIdx {
			minIdx = idx
			minN = i + 1
		}
