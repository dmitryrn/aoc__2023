...
impl From<&str> for Card {
    fn from(str: &str) -> Self {
        ...
        // 1234
        let bid: usize;
        if bid_str[3] == 0 && bid_str[2] == 0 && bid_str[1] == 0 {
            bid = std::str::from_utf8(&[bid_str[0]]).unwrap().parse().unwrap();
        } else if bid_str[3] == 0 && bid_str[2] == 0 {
            bid = std::str::from_utf8(&[bid_str[0], bid_str[1]])
                .unwrap()
                .parse()
                .unwrap();
        } else if bid_str[3] == 0 {
            bid = std::str::from_utf8(&[bid_str[0], bid_str[1], bid_str[2]])
                .unwrap()
                .parse()
                .unwrap();
        } else {
            bid = std::str::from_utf8(&[bid_str[0], bid_str[1], bid_str[2], bid_str[3]])
                .unwrap()
                .parse()
                .unwrap();
        }
        ...
        // 12233
        let mut acc = 0;
        for i in 0..(result_copied.len() - 1) {
            if result_copied[i] == result_copied[i + 1] {
                acc += 1;
            } else {
                if acc as isize - 1 >= 0 {
                    dup_info[acc - 1] += 1;
        ...
        let kind = || -> CardKind {
            if dup_info[3] > 0 {
                CardKind::FiveOfKind
            } else if dup_info[2] > 0 {
                CardKind::FourOfKind
            } else if dup_info[1] > 0 && dup_info[0] > 0 {
                CardKind::FullHouse
            } else if dup_info[1] > 0 {
                CardKind::ThreeOfKind
            } else if dup_info[0] > 1 {
                CardKind::TwoPair
            } else if dup_info[0] > 0 {
                CardKind::OnePair
            } else {
                CardKind::HighCard
            }
